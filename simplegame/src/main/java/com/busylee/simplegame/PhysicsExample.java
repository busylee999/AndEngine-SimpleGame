package com.busylee.simplegame;
...
/**
 * Created by busylee on 13.02.15.
 */
public class PhysicsExample extends SimpleBaseGameActivity implements IOnSceneTouchListener, SensorListener {
	...
	protected Scene onCreateScene() {
		...
		sensorManager.registerListener(this, Sensor.TYPE_GRAVITY);

		final VertexBufferObjectManager vertexBufferObjectManager = this.getVertexBufferObjectManager();
		final Rectangle ground = new Rectangle(CAMERA_WIDTH / 2, 0, CAMERA_WIDTH, 10, vertexBufferObjectManager);
...
//		this.mScene.attachChild(_Sprite);
	...
	private static Body createTriangleBody(final PhysicsWorld pPhysicsWorld, final IEntity pEntity, final BodyType pBodyType, final FixtureDef pFixtureDef) {
		...
		final float halfWidth = pEntity.getWidth() * 0.5f / PhysicsConstants.PIXEL_TO_METER_RATIO_DEFAULT;
		final float halfHeight = pEntity.getHeight() * 0.5f / PhysicsConstants.PIXEL_TO_METER_RATIO_DEFAULT;
	...
	private static Body createHexagonBody(final PhysicsWorld pPhysicsWorld, final IEntity pEntity, final BodyType pBodyType, final FixtureDef pFixtureDef) {
		...
		final float left = -halfWidth + 2.5f / PhysicsConstants.PIXEL_TO_METER_RATIO_DEFAULT;
		final float right = halfWidth - 2.5f / PhysicsConstants.PIXEL_TO_METER_RATIO_DEFAULT;
		final float higher = top + 8.25f / PhysicsConstants.PIXEL_TO_METER_RATIO_DEFAULT;
		final float lower = bottom - 8.25f / PhysicsConstants.PIXEL_TO_METER_RATIO_DEFAULT;
	...
	public void onSensorChanged(int i, float[] floats) {
		this.mPhysicsWorld.setGravity(new Vector2(-floats[0] / 6, -SensorManager.GRAVITY_EARTH));
	}
